...
export class PDFConversionService {
  ...
  ConvertFile(endpointSlug: string, file: File | null, outputFilename?: string, fileKey?: string, extraParams?: { [key: string]: string }): Observable<HttpEvent<any>> {
    ...
    return this.http.post(`${this.pdfToolsUrl}/${finalEndpoint}`, formData, {
      reportProgress: true,
      observe: 'events'
    });
  }

  ConvertMultipleFiles(endpointSlug: string, files: File[], outputFilename?: string): Observable<HttpEvent<any>> {
    const formData: FormData = new FormData();
  ...
  CompareFiles(endpointSlug: string, file1: File, file2: File, outputFilename?: string): Observable<HttpEvent<any>> {
    const formData: FormData = new FormData();
