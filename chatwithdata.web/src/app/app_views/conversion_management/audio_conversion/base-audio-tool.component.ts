...
import { AudioConversionService } from '../../../app_services/audio_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseAudioToolComponent {
    // Services
    protected audioService = inject(AudioConversionService);
    ...
    convert(): void {
        ...
        this.audioService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, undefined, this.fileKey)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.audioService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
