...
import { EbookConversionService } from '../../../app_services/ebook_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseEbookToolComponent {
    // Services
    protected ebookService = inject(EbookConversionService);
    ...
    // Protected method that child classes can override to provide extra parameters
    protected getExtraParams(): { [key: string]: string } | undefined {
        return undefined;
    }
    ...
    convert(): void {
        ...
        this.ebookService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey, this.getExtraParams())
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.ebookService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
