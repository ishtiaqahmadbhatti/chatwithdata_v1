...
import { ImageConversionService } from '../../../app_services/image_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseImageToolComponent {
    // Services
    protected imageService = inject(ImageConversionService);
    ...
    convert(): void {
        ...
        this.imageService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
                next: (event: HttpEvent<any>) => {
                    switch (event.type) {
                        ...
                        case HttpEventType.Response:
                            this.uploadProgress = 100; // Ensure complete
    ...
    saveFile(): void {
        ...
        this.imageService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
