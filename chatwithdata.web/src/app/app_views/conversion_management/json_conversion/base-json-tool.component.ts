...
export abstract class BaseJsonToolComponent {
    // Services
    protected jsonService = inject(JSONConversionService);
    ...
    convert(): void {
        ...
        this.jsonService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
                next: (event: HttpEvent<any>) => this.handleConversionEvent(event),
                error: (error) => this.handleError(error)
            });
    }

    protected handleConversionEvent(event: HttpEvent<any>): void {
        switch (event.type) {
