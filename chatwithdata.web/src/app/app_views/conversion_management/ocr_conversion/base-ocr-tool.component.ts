...
import { OCRConversionService } from '../../../app_services/ocr_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseOcrToolComponent {
    // Services
    protected ocrService = inject(OCRConversionService);
    ...
    convert(): void {
        ...
        this.ocrService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.ocrService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
