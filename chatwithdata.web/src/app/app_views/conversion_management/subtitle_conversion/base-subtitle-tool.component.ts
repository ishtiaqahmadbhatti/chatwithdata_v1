...
import { SubtitleConversionService } from '../../../app_services/subtitle_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseSubtitleToolComponent {
    // Services
    protected subtitleService = inject(SubtitleConversionService);
    ...
    getConversionOptions(): Record<string, string> {
        return {};
    }
    ...
    convert(): void {
        ...
        this.subtitleService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey, options)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.subtitleService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
