...
import { TextConversionService } from '../../../app_services/text_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseTextToolComponent {
    // Services
    protected textService = inject(TextConversionService);
    ...
    onFileSelected(file: File): void {
        if (file) {
            ...
            this.fileKey = ''; // Clear fileKey if a local file is selected
    ...
    convert(): void {
        ...
        this.textService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.textService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
