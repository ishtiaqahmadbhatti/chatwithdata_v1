...
import { VideoConversionService } from '../../../app_services/video_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseVideoToolComponent {
    // Services
    protected videoService = inject(VideoConversionService);
    ...
    convert(): void {
        ...
        this.videoService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.videoService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
