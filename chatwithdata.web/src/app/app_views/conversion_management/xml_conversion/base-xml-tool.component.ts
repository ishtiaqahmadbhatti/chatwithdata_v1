...
import { XMLConversionService } from '../../../app_services/xml_conversion.service';
import { ToastService } from '../../../app_services/toast';

@Component({
    template: ''
})
export abstract class BaseXmlToolComponent {
    // Services
    protected xmlService = inject(XMLConversionService);
    ...
    convert(): void {
        ...
        this.xmlService.ConvertFile(this.toolId, this.selectedFile, this.outputFilename, this.fileKey)
            .subscribe({
    ...
    saveFile(): void {
        ...
        this.xmlService.downloadFile(this.conversionResult.downloadUrl).subscribe({
            next: (blob) => {
